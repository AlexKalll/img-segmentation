...
def create_sidebar_controls():
    ...
    # Use form to prevent immediate reruns
    with st.sidebar.form("controls_form"):
        ...
        if image_source == "Sample Image":
            ...
            try:
                ...
                st.success(f"✅ Loaded {sample_choice} sample")
            ...
            if image_url:
                try:
                    ...
                    st.info("💡 Make sure the URL points to a valid image file (jpg, png, etc.)")
                    selected_image = load_sample_image()  # Fallback
    ...
    if run_segmentation:
        ...
        st.session_state.controls_state.update({
            'image_source': image_source,
            'sample_choice': sample_choice if image_source == "Sample Image" else st.session_state.controls_state['sample_choice'],
            'max_size': max_size,
            'k': k,
            'min_size': min_size,
            'sigma': sigma
    ...
    # Display current parameters outside the form
    st.sidebar.markdown("---")
    st.sidebar.subheader("📊 Current Settings")
    st.sidebar.write(f"**k:** {st.session_state.controls_state['k']}")
    st.sidebar.write(f"**min_size:** {st.session_state.controls_state['min_size']}")
    st.sidebar.write(f"**sigma:** {st.session_state.controls_state['sigma']}")
